...
export class LeadDatabase {
    ...
    addLead(input) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        const result = stmt.run(input.orgName, input.orgType, input.city, input.state, input.lastAuditDate || null, input.auditorName || null, input.source, input.estimatedValue || 0, input.notes || null);
    ...
    getPipeline(filter = {}) {
        ...
        return this.db.prepare(sql).all(...params).map((r) => this.rowToLead(r));
    }
    getStats() {
        const total = this.db.prepare("SELECT COUNT(*) as count FROM leads").get().count;
        const stageRows = this.db.prepare("SELECT stage, COUNT(*) as count FROM leads GROUP BY stage").all();
        const byStage = {};
        for (const row of stageRows)
            byStage[row.stage] = row.count;
        const totalValue = this.db.prepare("SELECT COALESCE(SUM(estimated_value), 0) as total FROM leads").get().total;
