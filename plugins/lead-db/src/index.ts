...
let db: LeadDatabase | null = null;
function getDb(): LeadDatabase {
  if (!db) db = new LeadDatabase(DB_PATH);
  return db;
}
...
export default defineToolPlugin({
  ...
  tools: (tool) => [
    tool({
      name: "add_lead",
      description:
        "Add a new organization to the lead pipeline. Checks for duplicates first. " +
    ...
    tool({
      name: "score_lead",
      description:
        "Calculate a lead's score (0-100) based on audit urgency, " +
    ...
    tool({
      name: "get_pipeline",
      description:
        "Get leads from the pipeline, optionally filtered by stage, state, or minimum score.",
      parameters: Type.Object({
